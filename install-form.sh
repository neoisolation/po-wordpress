...
GRANT ALL ON `postal`.* TO `postal`@`127.0.0.1` IDENTIFIED BY "$domainmsqlpass"; | mysql -u root;
GRANT ALL PRIVILEGES ON `postal-%` . * to `postal`@`127.0.0.1`  IDENTIFIED BY "$domainmsqlpass"; | mysql -u root;
