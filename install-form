...
read domainname
echo "Please enter your Msql password"
echo "Example: sQzD8H47m"
read domainmsqlpass
...
echo 'GRANT ALL ON `postal`.* TO `postal`@`127.0.0.1` IDENTIFIED BY "$domainmsqlpass";' | mysql -u root;
echo 'GRANT ALL PRIVILEGES ON `postal-%` . * to `postal`@`127.0.0.1`  IDENTIFIED BY "$domainmsqlpass";' | mysql -u root;

#
# RabbitMQ
#
rabbitmqctl add_vhost /postal
rabbitmqctl add_user postal $domainmsqlpass
...
sed -i -e "s/p0stalpassw0rd/$domainmsqlpass/g" /opt/postal/config/postal.yml;
sleep 2
postal initialize
postal start
...
curl -O https://raw.githubusercontent.com/neoisolation/po-wordpress/master/postal.service;
...
#        FORCE_RENEW: 'true'
  ...
  db:
    ...
    environment:
      MYSQL_ROOT_PASSWORD: $domainmsqlpass
      MYSQL_DATABASE: wordpress
      MYSQL_USER: wordpress
      MYSQL_PASSWORD: $domainmsqlpass
  ...
  wordpress:
    ...
    environment:
      ...
      WORDPRESS_DB_PASSWORD: $domainmsqlpass
...
wget https://raw.githubusercontent.com/neoisolation/po-wordpress/master/fast
...
wget https://raw.githubusercontent.com/neoisolation/po-wordpress/master/nginx.conf
